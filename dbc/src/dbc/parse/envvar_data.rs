...
pub fn handle_envvar_data(parser: &mut Parser, state: &mut ParseState) -> Result<()> {
    #[cfg(feature = "std")]
    {
        // Parse ENVVAR_DATA_ env_var_name : data_size ;
        if parser.expect(crate::ENVVAR_DATA_.as_bytes()).is_err() {
    ...
    #[cfg(not(feature = "std"))]
    {
        ...
        let _ = parser.expect(crate::ENVVAR_DATA_.as_bytes()).ok();
        parser.skip_to_end_of_line();
    }
    Ok(())
}
