...
pub fn handle_ba_def_def(parser: &mut Parser, state: &mut ParseState) -> Result<()> {
    #[cfg(feature = "std")]
    {
        ...
        if let Ok(attr_default) = (|| -> Result<crate::attributes::AttributeDefault> {
            ...
            state.attribute_defaults_buffer.push(attr_default);
        } else {
