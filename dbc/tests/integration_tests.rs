...
fn test_parse_simple_dbc() {
    let content = std::fs::read_to_string("tests/data/simple.dbc")
        .expect("Failed to read tests/data/simple.dbc");

    let dbc = Dbc::parse(&content).expect("Failed to parse simple.dbc");

    // Verify version
    assert_eq!(dbc.version().major(), 1);
    assert_eq!(dbc.version().minor(), Some(0));
...
fn test_parse_multiplexed_dbc() {
    let content = std::fs::read_to_string("tests/data/multiplexed.dbc")
        .expect("Failed to read tests/data/multiplexed.dbc");

    let dbc = Dbc::parse(&content).expect("Failed to parse multiplexed.dbc");

    // Verify version
    assert_eq!(dbc.version().major(), 2);
    assert_eq!(dbc.version().minor(), Some(1));
...
fn test_parse_minimal_dbc() {
    let content = std::fs::read_to_string("tests/data/minimal.dbc")
        .expect("Failed to read tests/data/minimal.dbc");
...
fn test_parse_extended_ids_dbc() {
    let content = std::fs::read_to_string("tests/data/extended_ids.dbc")
        .expect("Failed to read tests/data/extended_ids.dbc");

    let dbc = Dbc::parse(&content).expect("Failed to parse extended_ids.dbc");

    // Verify version
    assert_eq!(dbc.version().major(), 1);
    assert_eq!(dbc.version().minor(), Some(5));
...
fn test_parse_broadcast_signals_dbc() {
    let content = std::fs::read_to_string("tests/data/broadcast_signals.dbc")
        .expect("Failed to read tests/data/broadcast_signals.dbc");
...
fn test_parse_complete_dbc_file() {
    // Read the complete.dbc file
    let content = std::fs::read_to_string("tests/data/complete.dbc")
        .expect("Failed to read tests/data/complete.dbc");

    // Parse it with Dbc::parse
